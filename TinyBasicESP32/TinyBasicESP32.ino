...
Adafruit_STMPE610 ts = Adafruit_STMPE610(RT_CS);
...
enum {
  ...
  KW_NEXT, KW_LET,  KW_IF,    KW_GOTO,
  KW_GOSUB, KW_RETURN, KW_REM, KW_FOR,
  KW_INPUT, KW_PRINT, KW_POKE,  KW_STOP,
  KW_BYE,  KW_FILES, KW_DELETE, KW_MEM,
  KW_QMARK, KW_QUOTE, KW_AWRITE, KW_DWRITE,
  KW_DELAY, KW_END,  KW_RSEED, KW_CHAIN,
#ifdef ENABLE_TONES
...
void setup()
{
  ...
  if ( SD.exists( AUTORUN_FILE )) {
    program_end = program_start;
    fp = SD.open( AUTORUN_FILE );
    inStream = kStreamFile;
    inhibitOutput = true;
    runAfterLoad = true;
  }
